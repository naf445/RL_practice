...
class policy_iterator(object):
    ...
    def iterate(self, iteration_loops):
        ...
        current_policy.fill(1/self.nA)
        iterations_completed = 0
        policies_same = False
        list_of_policies = [current_policy.copy()]
        while not policies_same and iterations_completed < iteration_loops:
            ...
            new_policy = self.greedily_update_policy(updated_value_function=updated_value_function)
    ...
    def greedily_update_policy(self, updated_value_function):
        ...
        new_optimal_policy = np.eye(q_matrix.shape[-1])[np.argmax(q_matrix, axis=1)]
